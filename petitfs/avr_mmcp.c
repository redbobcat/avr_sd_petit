...
#define SELECT()	PORTD &= ~_BV(0)	/* CS = L */
#define	DESELECT()	PORTD |=  _BV(0)	/* CS = H */
#define	SELECTING	!(PORTD &  _BV(0))	/* CS status (true:CS low) */
#define	FORWARD(d)	xmit(d)				/* Data forwarding function (console out) */

void xmit (char);			/* suart.S: Send a byte via software UART */
void dly_100us (void) { _delay_us(100);}		/* usi.S: Delay 100 microseconds */
void init_spi (void) { spi_init();}		/* usi.S: Initialize MMC control ports */
void xmit_spi (BYTE d) {spi_byte(d);}		/* usi.S: Send a byte to the MMC */
BYTE rcv_spi (void) {return spi_byte(0xFF);}		/* usi.S: Send a 0xFF to the MMC and get the received byte */

void xmit (char i) //placeholder for xmit
	{
